...
const server = http.createServer((req, res) => {
    ...
    if(pathname === '/' || pathname === '/overview'){
        ...
        let cardTemplate = dataObj.map(product=>replaceTemplate(product, tempCard)).join('');
        //console.log(`cardTemplate: ${cardTemplate}, \n${typeof(cardTemplate)}`);
        const showtempOverview = tempOverview.replace('{%PRODUCT_CARDS%}', cardTemplate);
        res.end(showtempOverview);
    }
    //products page
    else if(pathname === '/product'){
        const indexObj = dataObj[query.id];
        console.log(query, " ", dataObj[query.id]);
